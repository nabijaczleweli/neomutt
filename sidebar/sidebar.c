...
void sb_set_open_mailbox(struct MuttWindow *win, struct Mailbox *m)
{
  ...
  {
    if (mutt_str_strcmp(wdata->entries[entry]->mailbox->realpath, m->realpath) == 0)
    {
 ...
 */
void sb_notify_mailbox(struct MuttWindow *win, struct SidebarWindowData *wdata,
                       struct Mailbox *m, bool created)
{
  if (!m)
    return;
 ...
 */
static void init_data(struct MuttWindow *win, struct SidebarWindowData *wdata)
{
  struct MailboxList ml = STAILQ_HEAD_INITIALIZER(ml);
  neomutt_mailboxlist_get_all(&ml, NeoMutt, MUTT_MAILBOX_ANY);
  struct MailboxNode *np = NULL;
  STAILQ_FOREACH(np, &ml, entries)
  {
    sb_notify_mailbox(win, wdata, np->mailbox, true);
  }
  neomutt_mailboxlist_clear(&ml);
}
