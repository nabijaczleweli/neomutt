 ...
 * @param ctx Threading context to finalize
 */
...
off_t mutt_set_vnum(struct Mailbox *m)
{
  ...
  for (int i = 0; i < m->msg_count; i++)
  {
    ...
    if (e->vnum >= 0)
    {
      ...
      m->v2r[m->vcount] = i;
      m->vcount++;
      vsize += e->content->length + e->content->offset - e->content->hdr_offset + padding;
      e->num_hidden = mutt_get_hidden(e);
    }
  }
  ...
  while (true)
  {
    ...
    if (e_cur)
    {
      if (flag & (MUTT_THREAD_COLLAPSE | MUTT_THREAD_UNCOLLAPSE))
      {
        ...
        if (reverse && (flag & MUTT_THREAD_COLLAPSE) && (e_cur->msgno < minmsgno) && e_cur->visible)
        {
          minmsgno = e_cur->msgno;
          final = e_cur->vnum;
        }
